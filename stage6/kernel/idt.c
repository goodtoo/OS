// kernel/idt.c — IDT + PIC 重映射 + PIT + C 层 ISR
#include <stdint.h>
#include "sched.h"
...
/* 统一的 C 层中断处理：现在多了一个参数 esp_ptr */
void isr_handler(uint32_t vec, uint32_t* esp_ptr) {
    volatile uint16_t *vm = (uint16_t*)0xB8000;
    static int pos = 160;

    if (vec == 32){                 // PIT tick
        outb(0x20, 0x20);           // 先EOI（避免丢中断）
        vm[pos++] = 0x0E00 | '*';   // 心跳
        sched_on_tick(esp_ptr);     // ★ 通知调度器
    } else if (vec == 33){          // 键盘
        (void)inb(0x60);
        outb(0x20, 0x20);
        vm[pos++] = 0x0A00 | '#';
...
static void set_gate(int n, uint32_t h){
    idt[n].off_lo = h & 0xFFFF; idt[n].sel = 0x08; idt[n].zero=0; idt[n].type=0x8E; idt[n].off_hi=(h>>16)&0xFFFF;
}
void idt_init(void){
